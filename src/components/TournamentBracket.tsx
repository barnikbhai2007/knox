...
export default function TournamentBracket({ brackets }: { brackets: any[] }) {
  const [bracketImageUrl, setBracketImageUrl] = useState<string | null>(null);

  useEffect(() => {
    const imgBracket = brackets.find(b => b.round === 'BRACKET_IMAGE');
    if (imgBracket && imgBracket.fc_team1) {
      setBracketImageUrl(imgBracket.fc_team1);
    } else {
      setBracketImageUrl(null);
    }
  }, [brackets]);
  ...
  if (activeRounds.length === 0 && !bracketImageUrl) return (
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-[100vw] overflow-x-auto">
      {bracketImageUrl && (
         <div className="w-full flex justify-center mb-8">
            <img src={bracketImageUrl} alt="Tournament Bracket" className="w-full max-w-4xl rounded-2xl shadow-2xl border border-white/10" />
         </div>
      )}

      {activeRounds.length > 0 && (
        <div className="flex bg-gray-950/50 p-6 rounded-2xl gap-8 items-stretch min-w-max border border-white/5 pt-12 md:scale-100 scale-90 origin-top-left md:origin-top">
